...
def lista(update, context):
    ...
    try:
        entro = False
        if update.effective_chat.id == 805954751 or context.args[0] == 'marioAreta100':
            entro = True
            datos = convertirTablaADiccionario(c, 'gente')

            personas = ''
            for name, value in datos.items():
                telegramID = value[0]
                telegramName = value[1]
                personas = personas + " " + \
                    str(telegramID) + " " + str(telegramName) + "\n"
            print(personas)
            text = '<b> La cantidad de personas registradas son: </b>' + \
                str(len(datos)) + "\n\n" + personas
            context.bot.send_message(
                chat_id=update.effective_chat.id, text=text, parse_mode=telegram.ParseMode.HTML)
            return 0
        ...
        pass
    if update.effective_chat.id != 805954751 and entro == False:
        context.bot.send_message(
            chat_id=update.effective_chat.id, text='No such command')
...
def main():
    try:
        ...
        send_message_handler = CommandHandler('send_message', send_message)
        lista_handler = CommandHandler('lista', lista)
